...
export const getKeyConfigurationFromEnvironment = (
  ...
  const keyConfiguration: KeyConfiguration = {
    ...
    azureEmbeddingDeploymentName: configService.get('AZURE_OPENAI_API_EMBEDDINGS_DEPLOYMENT_NAME'),
    basePath: configService.get('OPENAI_BASE_URL'),
  }
...
const validateKeyConfiguration = (keyConfiguration: KeyConfiguration): void => {
  switch (keyConfiguration.apiType) {
    ...
    case ModelType.AZURE_OPENAI:
      ...
      if (!keyConfiguration.azureApiKey) missingKeys.push('AZURE_OPENAI_API_KEY')
      if (!keyConfiguration.azureInstanceName) missingKeys.push('AZURE_OPENAI_API_INSTANCE_NAME')
      if (!keyConfiguration.azureApiVersion) missingKeys.push('AZURE_OPENAI_API_VERSION')
      ...
      if (missingKeys.length > 0) {
        throw new Error(`Expected environment values: ${missingKeys.join(', ')}`)
      }
      break
  }
}
